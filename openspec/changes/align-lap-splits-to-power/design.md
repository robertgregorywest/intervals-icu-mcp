...
This matters beyond convenience. The prototype recovered 8.33–8.37 m per assumed 250 m lap, against the **8.526 m/rev** §1 gives for the 110" (65×16) gear — about 2% short. §1 already resolves the gear-inches-versus-development trap, so this is not that trap: it is either a different gear that day or a path ~2% longer than the measurement line, and the fit is what makes the question askable. A supplied constant would have absorbed it silently. Agreement across independent runs then serves as a free consistency check: four runs landing within 0.45% is strong evidence the alignment is right, and a run disagreeing with its siblings is evidence it is not.
