...
- The **effective development** the fit recovers — metres of assumed lap distance per crank revolution — is returned as a first-class output, and doubles as a cross-run consistency check. It is a measurement where there is currently an assumption, and the repo has paid for assumed constants before (`track-context.md` §6, where over-assuming one by ~6% sustained a false "the SRM reads low" conclusion for several analysis cycles). On 2026-08-08 it lands ~2% below the 8.526 m/rev §1 gives for the 110" gear — a real, unresolved question about the gear or the line ridden, now askable.
