...
**Workout brief**:
The distilled decision a coaching skill hands to its forked build skill: what to build, the context that shapes it (block, constraints, recent load) and when. One per discipline, each with a single caller — `plan-workout` → `compose-workout` (adds the library item or compose fresh, and the anchors) and `strength-training` → `compose-strength-session` (the **strength brief**; adds the template tier). Each is defined once, in its build skill's "Your input"; the caller points there rather than restating it. The decision is made on the coaching thread; the brief carries it to the build.
