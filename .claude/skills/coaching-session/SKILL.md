...
1. **Window** — from `reviewed-through` in the log header to today; see the table in [coaching-log-format.md](coaching-log-format.md) for a missing, stale, or too-recent watermark. Settle the window from the header alone — a watermark less than a day old skips the review here, with the watermark left alone.
2. **Call `get_execution_digest`** with that window, in the session-start batch. It selects the window's key sessions from the planned side, runs both lenses, and returns the work steps that missed their prescription plus the window's middle-band dose. It is synchronous and deterministic: there is nothing to wait for and no fork to barrier on.
3. **Interpret it here**, against [execution-review-lenses.md](execution-review-lenses.md) — read that file when a review is actually running. The digest reports deltas; deltas are not findings. What it cannot know, you can: whether a flagged rep recurs (against the log's **open threads**, which is why a pattern first seen in an earlier window still counts), whether a test's overshoot is the test working, and what any of it should change.
4. **Done = every key session dispositioned.** Each lands on **reported** (it recurs, within the window or against an open thread) or **held** (seen once — not raised now, ready if asked). A window where everything landed as prescribed gets _one line_ plus the middle-band figure, not silence. `status: "reviewed"` carries `reviewedThrough` — the date the watermark advances to at the log checkpoint, never here. `status: "skipped"` (no key session in the window) leaves the watermark alone; say it was skipped.

**A narrow request still gets the review.** If the athlete opens with something specific ("move Thursday's session"), run the digest alongside it and raise findings only where they bear on the request.
...
| Execution review     | `get_execution_digest` over the watermark window, read against [execution-review-lenses.md](execution-review-lenses.md) — see _Execution review_ above                                             |
...
**Forks run in the background.** `ride-analysis` and the `compose-workout` /
...
- **Two things the forecast does not model.** Strength contributes no load (the platform assigns none either), so a week with two gym sessions is under-read on fatigue. And it says what the sessions would cost _if ridden as written_ — whether they will be is what the execution review answers.
