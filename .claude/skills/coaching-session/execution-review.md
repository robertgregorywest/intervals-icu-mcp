...
**Only work steps carry the session's intent.** A verdict on a warm-up, a recovery step, or a cool-down is not a finding — see the worked example below, where 5 of 8 steps return `under` and only one is real.

**Deriving the role.** No marker exists on the planned side, and the delivered `type` field is auto-detected and unreliable — a probed track session can report a `RECOVERY` interval averaging well above threshold power. So derive it from two signals _used together_:
...
**Range targets carry no tolerance.** The `tolerance` parameter applies to point targets only; a range-target step is judged on its own band, so a delivery a few watts outside it returns a directional verdict with no allowance. Small deltas on range targets are noise.
...
- L3 −1437 s — the reps were ridden slightly above the prescribed range's midpoint, so their seconds landed in the partition's next zone up instead. The work happened; the frame moved it.

**What's real.** One work step: rep 1, under its prescribed band.
