...
- **A structural token in a label is a parse error.** Step and repeat-block labels are plain text: no duration (`60s`), watts (`350w`), percentage (`75%`), cadence (`90rpm`) or zone (`Z2`) token. Intervals.icu re-reads the rendered line and eats them as structure, so the file looks right while the head unit is wrong. Incidental digits are fine (`20min effort`). Put numeric detail in the prose.
- **`tests/fixtures/rendered-templates.txt`** is the committed render of every template at MAP=415/FTP=290. Edit a template on purpose → `npm test -- -u`, and the snapshot diff is the review artifact for the session. An unexpected diff means a renderer change altered a workout.
